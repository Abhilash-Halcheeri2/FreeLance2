...
import { ControlType, GridType, SubAreaNames, Timeouts, generateRandomString } from '../constants/Commons/commons'
...
import { NavigateToSubArea, clearCache, loginToAccount } from '../utility/helper'
...
test.describe('Admin User TestCase Runner', () => {
  ...
  test.beforeEach(async ({ context, page }) => {
    ...
    await loginToAccount(page, Orgisationdetails.SuperAdmin_UserName, Orgisationdetails.SuperAdmin_UserPassword)
  })
  ...
  test('TC008:validate saved booking records without search', async ({ page }) => {
    await NavigateToSubArea(page, SubAreaNames.savedBookingRecords)
    await ValidatesavedBookingRecordsGridPageWithoutSearch(page)
  })

  test('TC009:Search and validate the saved booking record', async ({ page }) => {
    await NavigateToSubArea(page, SubAreaNames.savedBookingRecords)
    await SearchRecordsAndValidateInGrid(page, SavedBookingRecorsPage.RecordSearchBar, 'adc')
  })

  test('TC010:search and validate the advanced search result,DateOfBirth', async ({ page }) => {
    await NavigateToSubArea(page, SubAreaNames.savedBookingRecords)
    await Click_AdvanceSearch_btn(page)
    await selectAdvancedSearchFieldValue(page, AdvancedSearchfieldValuesDropdown_opt.dateOfBirth)
  ...
  test('TC011:savedBookingRecords: search and validate the advanced search result', async ({ page }) => {
    await NavigateToSubArea(page, SubAreaNames.savedBookingRecords)
    await Click_AdvanceSearch_btn(page)
    await selectAdvancedSearchFieldValue(page, AdvancedSearchfieldValuesDropdown_opt.first)
    await selectAdvancedSearchOperator(
      page,
  ...
  test.only('TC012:Create sex_offender record', async ({ page }) => {
    test.setTimeout(120000)
    await NavigateToSubArea(page, SubAreaNames.Sex_Offender)
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_FirstName, generateRandomString(5))
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_LastName, generateRandomString(5))
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_Height, '506')
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_Weight, '206')

    await selectOptionsFromBiographicsDD(page, Biographics_Dropdown_fields.DD_Gender, DD_GenderOfVictim_List.female)
    await selectOptionsFromBiographicsDD(page, Biographics_Dropdown_fields.DD_Race, DD_Race_List.asian)

    await selectOptionsFromBiographicsDD(page, Biographics_Dropdown_fields.DD_EyeColor, DD_EyeColor_List.blue)
    await selectOptionsFromBiographicsDD(page, Biographics_Dropdown_fields.DD_HairColor, DD_HairColor_List.black)
    await selectOptionsFromBiographicsDD(
      page,
      Biographics_Dropdown_fields.DD_PlaceOfBirth,
      DD_PlaceOfBirth_Opt_List.afghanistan
    )
    await BiographicsFormDateSelect(page, TypeOfDatePickers.DateOfBirth, '2000/11/15')
    await selectOptionsFromBiographicsDD(
      page,
      Biographics_Dropdown_fields.DD_RegistrantCounty,
      DD_RegistrantCounty_Opt_List.apache
    )
    await BiographicsFormDateSelect(page, TypeOfDatePickers.ConfinementReleaseDate, '2025/10/11')
    await selectOptionsFromBiographicsDD(
      page,
    ...
    await selectOptionsFromBiographicsDD(page, Biographics_Dropdown_fields.DD_Class, DD_Class_opt_List.felony)
    await BiographicsFormDateSelect(page, TypeOfDatePickers.ArrestDate, '2020/05/15')
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_ArrestAgency, generateRandomString(6))
    await selectOptionsFromBiographicsDD(page, Biographics_Dropdown_fields.DD_ArrestState, DD_ArrestState_List.alabama)
    await BiographicsFormDateSelect(page, TypeOfDatePickers.ConvictionDate, '2020/08/12')
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_CourtName, generateRandomString(5))
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_ADCNumber, generateRandomString(6))
    await setBiographicsfieldValueOnForm(
      page,
    ...
    await saveAndSubmitBiographicsForm(page)
  })
  ...
  test('TC0015:transaction Viewer AdvancedSearch Search', async ({ page }) => {
    ...
    await ClickOnAdvancedSearchResult(page)
    await validateTransactionViewerAdvancedsearchRecord(page, TransactionViewerTableHeader.pCN, '00890')
  })
  ...
  test('Test0016: Create ADC intake Record', async ({ page }) => {
    ...
    await BiographicsFormDateSelect(page, TypeOfDatePickers.DateOfBirth, '2000/11/15')
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_ADCNumber, generateRandomString(6))
    await BiographicsFormDateSelect(page, TypeOfDatePickers.SentenceDate, '2000/10/10')
    await selectOptionsFromBiographicsDD(
      page,
      Biographics_Dropdown_fields.DD_CourtOfCommitment,
      DD_CourtofCommitment_List.APACHE_COUNTY
    )
    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_CourtRecordNumber, '123456')
    await selectOptionsFromBiographicsDD(
      page,
      Biographics_Dropdown_fields.DD_PrimaryCharge,
      DD_PrimaryCharge_List.RESTRICTED_DESIGNATION_VIOL
    )
    await BiographicsFormDateSelect(page, TypeOfDatePickers.ChargesArrestDate, '2020/05/15')
  ...
  test('Test0017: Create Crim_JusticeApplicant Record', async ({ page }) => {
    ...
    await selectOptionsFromBiographicsDD(page, Biographics_Dropdown_fields.DD_EyeColor, DD_EyeColor_List.blue)
    await selectOptionsFromBiographicsDD(page, Biographics_Dropdown_fields.DD_HairColor, DD_HairColor_List.black)
    await BiographicsFormDateSelect(page, TypeOfDatePickers.crimJDOB, '2000/11/15')

    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_Description, generateRandomString(6))

    await setBiographicsfieldValueOnForm(page, Biographics_form_fields.txt_box_Authority, generateRandomString(6))
  ...
  test('tc_00110:Delete record from saved booking records', async ({ page }) => {
  ...
  test('upload cards', async ({ page }) => {
    const card = './tests/Attachments/ChemCorp.dgn'
    await NavigateToSubArea(page, SubAreaNames.cards)
    await uploadCard(page, 'Test', 'ADC Intake', card)
  })
