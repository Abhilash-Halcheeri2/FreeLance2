...
export async function navigateToBookingTab(page: Page, tabName: string) {
  ...
  const isTabVisible = await page
    .locator(stringFormat(New_BookingRecord_Selectors.Form_TabList, tabName))
    .isVisible();
...
export async function saveAndSubmitBiographicsForm(page: Page) {
  ...
  await page.waitForSelector("[class='chakra-stack css-1xbaot1']");
  await page
    .locator(
      stringFormat(
        Sex_Offender_Selectors.buttonLocator,
...
export async function BiographicsFormDateSelect(
  ...
  console.log("Biographics_Form Date " + targetDate);
